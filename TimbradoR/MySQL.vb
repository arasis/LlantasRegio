...
Public Class MySQLQ
    ...
    Private _ComandoSP As MySqlCommand
    ...
    Public Function EjecutaSQL(ByVal SQL As String) As Boolean
        'Dim Conexion As MySqlConnection = New MySqlConnection(_ConnStr)
        'Try
        '    Conexion.Open()
        '    _QuerySQL = SQL
        '    Dim sqlData As MySqlDataAdapter = New MySqlDataAdapter(_QuerySQL, Conexion)
        ...
        ' Dim Conexion As SqlConnection = New SqlConnection(conectar)
        Try
            Dim conectar As New SqlConnection(DB.obtenerConexionDB)

            conectar.Open()
            _QuerySQL = SQL
            Dim sqlData As SqlDataAdapter = New SqlDataAdapter(_QuerySQL, conectar)
            Dim ds As DataSet = New DataSet()
            Dim cuantas As Integer = sqlData.Fill(ds)
            conectar.Close()
            If ds.Tables.Count > 0 Then _DatosRegresados = ds.Tables(0)
    ...
    Public Sub CreaNuevoSP(ByVal sp As String)
        _ComandoSP = New MySqlCommand()
        _ComandoSP.CommandText = sp
        _ComandoSP.CommandType = System.Data.CommandType.StoredProcedure
    End Sub
    ...
    Public Sub AgregaParam(ByVal nombre As String, ByVal tipo As MySql.Data.MySqlClient.MySqlDbType, ByVal valor As Object)
        Dim param As MySqlParameter = New MySqlParameter("?" & nombre, tipo)
        param.Value = valor
    ...
    Public Function EjecutaSP() As Boolean
        Dim Conn As MySqlConnection = New MySqlConnection(_ConnStr)
        Try
            Conn.Open()
            _ComandoSP.Connection = Conn
            Dim sqa As MySqlDataAdapter = New MySqlDataAdapter(_ComandoSP)
    ...
    Public Sub AgregaParamReturn(ByVal nombre As String)
        _ComandoSP.Parameters.Add("@" & nombre, MySqlDbType.Int32)
        _ComandoSP.Parameters("@" & nombre).Direction = ParameterDirection.Output
    End Sub

    Public Sub AgregaParamReturn(ByVal nombre As String, ByVal type As MySqlDbType)
